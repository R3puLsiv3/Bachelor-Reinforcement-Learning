...
class UniformMemory:
    def __init__(self, state_size, action_size, memory_size):
        ...
        self.state = torch.empty(memory_size, state_size, dtype=torch.float)
        self.action = torch.empty(memory_size, action_size, dtype=torch.float)
        self.reward = torch.empty(memory_size, dtype=torch.float)
        self.next_state = torch.empty(memory_size, state_size, dtype=torch.float)
...
class PrioritizedExperienceReplayMemory:
    ...
    def sample(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            cum_sum = random.uniform(a, b)
            # sample_idx is a sample index in buffer, needed further to sample actual transitions
            # tree_idx is a index of a sample in the tree, needed further to update priorities
            tree_idx, priority, sample_idx = self.tree.get(cum_sum)
